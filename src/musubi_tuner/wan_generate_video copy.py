...
def setup_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    from musubi_tuner.wan.configs import WAN_CONFIGS as _WAN_CFGS
    _cfg = _WAN_CFGS[args.task]
    logger.info(
        f"WAN task: {args.task} | Using WAN {'2.2' if getattr(_cfg, 'v2_2', False) else '2.1'} | "
        f"i2v: {getattr(_cfg, 'i2v', False)} | Fun-Control: {getattr(_cfg, 'is_fun_control', False)}"
    )
...
def load_dit_model(
    ...
    # if we only want to save the model, save and skip the rest
    if args.save_merged_model:
        # Save merged model for non-LyCORIS path as well (LoRA already merged into state_dict above)
